...
import org.jivesoftware.openfire.dbn.dbnRedis.DbnRedisServerOfSource;
import org.jivesoftware.openfire.dbn.dbnUtils.DbnUtils;
    ...
    private void process(Presence presence, ClientSession session) throws UnauthorizedException, PacketException {
        try {
            ...
            if (type == null) {
                ...
                LocalSession test = (LocalSession) sessionManager.getSession(presence.getFrom());
                int s = test.getStatus();
...
//				JedisPool pool = DbnRedisServerOfSource.getJedis();
//				Jedis jedis = pool.getResource();
//				try {
//					
//					for (Message message : messages) {
//						session.process(message);
//						Element dbnElement = message.getChildElement(DbnUtils.DBN_ELEMENT_NAME,DbnUtils.DBN_NAMESPACE);
//						if (null != dbnElement && !message.getTo().toBareJID().contains("admin") && !"0".equals(message.getBody())) {
//							Date creationDate = new Date(System.currentTimeMillis());
//							String delayMessage = message.toXML();
//							if (!delayMessage.contains("delay")) {
//								// Add a delayed delivery (XEP-0203) element to
//								// the message.
...
//								// Add a legacy delayed delivery (XEP-0091)
//								// element to the message. XEP is obsolete and
//								// support should be dropped in future.
//								delay = message.addChildElement("x","jabber:x:delay");
//								delay.addAttribute("from", XMPPServer.getInstance().getServerInfo().getXMPPDomain());
//								delay.addAttribute("stamp", XMPPDateTimeFormat.formatOld(creationDate));
//							}
//							Long time = System.currentTimeMillis();
//							jedis.zadd(DbnUtils.OPENFIRE+DbnUtils.NXIN_OFFLINE_MAP, time,message.getID());
//							jedis.set(DbnUtils.OPENFIRE+message.getID(), message.toString());
//							jedis.set(DbnUtils.OPENFIRE+DbnUtils.REDIS_TIME+message.getID(),time+"");
//						}
//					}
//				} catch (Exception e) {
//					Log.error("PresenceUpdateHandler is error :" + e);
//				} finally {
